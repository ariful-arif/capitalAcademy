...
<div class="container mt-4">
    <h2 class="mb-4">mcqs for Lesson: {{ $lesson->title }}</h2>
        ...
        <div class="card-body">
            ...
            @if ($lesson->attachment)
                <form id="generatemcqsForm">
                    ...
                    <button type="submit" class="btn btn-primary mt-3">Generate mcqs from PDF</button>
                </form>
            @else
    ...
    <form id="mcqForm">
        ...
        <div id="mcqContainer" class="row g-3 mb-4">
            @if (is_array($lesson->mcq_question) && count($lesson->mcq_question))
                @foreach ($lesson->mcq_question as $index => $mcq)
...
<script>
    ...
    document.getElementById('generatemcqsForm').addEventListener('submit', function(e) {
        ...
        container.innerHTML = '<div class="text-muted">Generating mcqs...</div>';

        fetch("{{ route('admin.ai.mcqs') }}", {
                method: "POST",
                headers: {
