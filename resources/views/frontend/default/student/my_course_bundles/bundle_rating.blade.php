        ...
        <form action="{{ route('course.bundle.rating.store', ['bundle_id' => $bundle->id]) }}" method="POST">
            ...
            <div class="form-group mb-3">
                <select id="user_bundle_rating" name="rating" class="form-control" required style="background: #261954; color: #fff;">
                    <option value="">{{ __('select_rating') }}</option>
                    @for ($i = 1; $i <= 5; $i++)
                        <option value="{{ $i }}">{{ $i }} {{ __('out_of') }} 5</option>
                    @endfor
                </select>
            </div>
            <div class="form-group mb-3">
                <textarea id="user_bundle_comment" class="form-control" name="comment" style="background: #261954; color: #fff;"></textarea>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <button type="submit" class="btn btn-primary float-end">{{ __('publish_rating') }}</button>
                </div>
            </div>
        </form>
    </div>
