        ...
        <div class="ol-card-body my-3 py-4 px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a href="{{ route('organization.users.create') }}"
                    ...
                    <span>{{ get_phrase('Add new User') }}</span>
                </a>
            </div>
        </div>
    </div>
    <div class="ol-card p-4">
        <div class="ol-card-body">
            ...
            <div class="row print-d-none mb-3 mt-3 row-gap-3">
                ...
                <div class="col-md-6">
                    <form class="form-inline" action="{{ route('organization.users') }}" method="get">
            ...
            <div class="row mt-4">
                <div class="col-md-12">
                    <!-- Table -->
                    @if (count($organization_users) > 0)
                        <div
                            class="admin-tInfo-pagi d-flex justify-content-between justify-content-center align-items-center flex-wrap gr-15">
                            <p class="admin-tInfo">
                                {{ get_phrase('Showing') . ' ' . count($organization_users) . ' ' . get_phrase('of') . ' ' . $organization_users->total() . ' ' . get_phrase('data') }}
                            </p>
                        </div>
                        <div class="table-responsive course_list" id="course_list">
                            <table class="table eTable eTable-2 print-table">
                                ...
                                <tbody>
                                    @foreach ($organization_users as $key => $row)
                                        <tr>
                                            <th scope="row">
                                            ...
                                            <td>
                                                @if ($row->status == 1)
                                                    <span
                                                        class="badge bg-active">{{ get_phrase(ucfirst('active')) }}</span>
                                                @else
                                                    <span
                                                        class="badge bg-inactive">{{ get_phrase(ucfirst('inactive')) }}</span>
                                                @endif
                                            </td>
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li>
                                                            <a class="dropdown-item"
                                                                href="{{ route('organization.users.edit', $row->id) }}">{{ get_phrase('Edit') }}</a>
                                                        </li>

                                                        <li>
                                                            <a class="dropdown-item"
                                                                onclick="confirmModal('{{ route('organization.users.delete', $row->id) }}')"
