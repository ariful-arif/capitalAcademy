                    ...
                    <form action="{{ route('admin.course.bundle.update', $course_bundle->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="fpb-7 mb-3">
                                ...
                                <textarea name="short_description" id="short_description" rows="4" class="form-control ol-form-control" placeholder="{{ get_phrase('Enter short description') }}">{{ $course_bundle->short_description }}</textarea>
                            </div>

                            <div class="mb-3">
                                <label for="bundle_category_id" class="form-label ol-form-label">{{ get_phrase('Bundle category') }}</label>
                                <select class="form-control ol-form-control ol-select2" name="bundle_category_id" id="bundle_category_id" required>
                                    <option value="">{{ get_phrase('Select category') }}</option>
                                    @foreach (App\Models\Bundle_category::get() as $bundle_category)
                                        <option value="{{ $bundle_category->id }}" @if($bundle_category->id == $course_bundle->bundle_category_id) selected @endif>{{ $bundle_category->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                           ...
                           <div class="fpb-7 mb-3">
                            ...
                            <div class="fpb-7 mb-3">
                                <label class="form-label ol-form-label" for="trailer_video_link">{{ get_phrase('Trailer video link') }} <small>(Youtube, Vimeo, HTML5)</small></label>
                                <input type="link" class="form-control ol-form-control" value="{{$course_bundle->trailer_video_link}}" name="trailer_video_link" id="trailer_video_link" placeholder="http://youtube.com/watch?v=xxx" required>
                            </div>
