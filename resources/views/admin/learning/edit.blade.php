...
@push('title', get_phrase('Update learning'))
@push('meta')@endpush
@push('css')@endpush
@section('content')
    <style>
        ...
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                <h4 class="title fs-16px">
                    ...
                    <span>{{ get_phrase('Update learning') }}</span>
                </h4>
                <a href="{{ route('admin.learnings') }}" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
        ...
        <div class="col-md-10">
            <div class="ol-card p-4">
                <div class="ol-card-body">
                    <form action="{{ route('admin.learning.update', $learning_data->id) }}" method="post" enctype="multipart/form-data">@csrf
                        <div class="fpb-7 mb-3">
                            <label class="form-label ol-form-label" for="title">{{ get_phrase('Title') }}</label>
                            <input type="text" class="form-control ol-form-control" name="title" id="title" placeholder="{{ get_phrase('Enter learning title') }}" value="{{ $learning_data->title }}" required>
                        </div>

                        <div class="fpb-7 mb-3">
                            <label class="form-label ol-form-label" for="learning_category_id">{{ get_phrase('Category') }}</label>
                            <select class="form-control ol-form-control ol-select2" data-toggle="select2" name="category_id" id="learning_category_id" required>
                                <option value="">{{ get_phrase('Select a category') }}</option>
                                @foreach ($category as $row)
                                    <option value="{{ $row->id }}"@if ($learning_data->category_id == $row->id) selected @endif>
                                        {{ $row->title }}</option>
                                @endforeach
                        ...
                        <div class="fpb-7 mb-3">
                            ...
                            <input type="text" name="keywords" value="{{ $learning_data->keywords }}" class="tagify ol-form-control w-100" data-role="tagsinput">
                        ...
                        <div class="fpb-7 mb-3">
                            ...
                            <textarea name="description" class="form-control ol-form-control text_editor" id="summernote-basic">{!! removeScripts($learning_data->description) !!}</textarea>
                        </div>

                        <div class="row">
                            <div class="col-md-6 fpb-7">
                                <label class="form-label ol-form-label" for="banner">{{ get_phrase('learning banner') }}</label>
                                <div class="image_preview">
                                    <img src="{{ asset($learning_data->banner) }}" id="preview_banner" alt="learning-banner">
                                </div>
                                <input type="file" name="banner" id="banner" class="form-control image-upload" accept="image/*">
                            </div>

                            <div class="col-md-6 fpb-7 ">
                                <label class="form-label ol-form-label" for="thumbnail">{{ get_phrase('learning thumbnail') }}</label>
                                <div class="image_preview">
                                    <img src="{{ asset($learning_data->thumbnail) }}" id="preview_thumbnail" alt="learning-thumbnail">
                                </div>
                        ...
                        <div class="fpb-7 mb-3 mt-3">
                            ...
                            <div class="d-flex gap-4">
                                <div class="d-flex align-items-center gap-2">
                                    <input type="radio" id="mark_yes" value="1" name="is_popular" @if ($learning_data->is_popular == 1) checked @endif>
                                    <label for="mark_yes">{{ get_phrase('Yes') }}</label>
                                </div>

                                <div class="d-flex align-items-center gap-2">
                                    <input type="radio" id="mark_no" value="0" name="is_popular" @if ($learning_data->is_popular == 0) checked @endif>
                        ...
                        {{-- @php
                            $seo_meta_tag = App\Models\SeoField::where('learning_id', $learning_data->id)->firstOrNew();
                        @endphp
                        <div class="fpb-7 mb-3">
                        ...
                        <div class="fpb-7 mb-3">
                            <button type="submit" class="ol-btn-primary">{{ get_phrase('Update learning') }}</button>
                        </div>
                    </form>
                </div>
            </div>
