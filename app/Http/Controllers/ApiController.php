...
class ApiController extends Controller
{
    ...
    public function update_userdata(Request $request)
    {
        ...
        try {
            ...
            // Prepare data, keeping old values if input is empty
            $data = [
                'name' => $request->filled('name') ? htmlspecialchars($request->name, ENT_QUOTES, 'UTF-8') : $existingUser->name,
            ...
            if ($request->hasFile('photo')) {
                ...
                $path = 'public/uploads/users/' . $user->role . '/' . $file_name;

                // Assuming FileUploader::upload() uploads the file successfully
...
// certified members
    ...
    public function ai_chat(Request $request)
    {
        ...
        // Hardcoded API Key (Replace this with your real OpenAI API Key)
        // $apiKey = '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';
        // $apiKey = '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';
        // premium key
        $apiKey = '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';
    ...
    // ai flashcards api
    public function ai_flashCards1(Request $request)
    {
        $request->validate([
        ...
        $cards = explode("Q:", $flashcards);
        $flashcardArray = [];

        foreach ($cards as $card) {
            if (trim($card)) {
                [$question, $answer] = explode("A:", $card);
                $flashcardArray[] = [
    ...
    public function ai_flashCards2(Request $request)
    {
        $request->validate([
            'pdf' => 'required|mimes:pdf',
        ]);
        ...
        if ($response->successful()) {
            ...
            // Now parse the flashcards from the raw text
            $flashcards = [];

            preg_match_all('/\*\*Flashcard \d+\*\*\s+Q: (.*?)\s+A: (.*?)\s+(?:---|$)/s', $rawContent, $matches, PREG_SET_ORDER);
    ...
    public function ai_flashCards3(Request $request)
    {
        $request->validate([
            'pdf' => 'required|mimes:pdf',
        ]);
        ...
        $response = Http::withHeaders([
            ...
            'Content-Type' => 'application/json',
                    ...
                    'model' => 'gpt-4o-mini',
                    'messages' => [
                        ['role' => 'system', 'content' => 'You are a flashcard generator. Format the flashcards like:\n\nFlashcard 1\nQ: question\nA: answer\n---'],
                        ['role' => 'user', 'content' => $prompt],
                    ],
                ]);
        ...
        // Get the OpenAI generated content
        $rawFlashcards = $response['choices'][0]['message']['content'];

        // Parse the flashcards from text into array
        $cards = [];
        preg_match_all('/\*\*Flashcard \d+\*\*\s+Q:\s*(.*?)\s+A:\s*(.*?)\s+---/s', $rawFlashcards, $matches, PREG_SET_ORDER);
    ...
    public function ai_flashCards4(Request $request)
    {
        $request->validate([
            'pdf' => 'required|mimes:pdf',
        ]);
        ...
        $response = Http::withHeaders([
            ...
            'Content-Type' => 'application/json',
                    ...
                    'model' => 'gpt-4o-mini',
                    'messages' => [
                        ['role' => 'system', 'content' => 'You are a flashcard generator. Format your response as plain text only.'],
                        ['role' => 'user', 'content' => $prompt],
                    ],
                ]);
        ...
        // Return plain text flashcards as received
        $formattedFlashcards = $response['choices'][0]['message']['content'];

        return response($formattedFlashcards, 200)
            ->header('Content-Type', 'text/plain');
    }


    public function ai_flashCards5(Request $request)
    {
        $request->validate([
            'pdf' => 'required|mimes:pdf',
        ]);
        ...
        $response = Http::withHeaders([
            ...
            'Content-Type' => 'application/json',
                    ...
                    'model' => 'gpt-4o-mini',
                    'messages' => [
                        ['role' => 'system', 'content' => 'You are a flashcard generator.Generate minimum 100 and maximum 500 if possible and format your response as plain text, using "Flashcard 1:", "Question 1:", and "Answer 1:" etc.'],
                        ['role' => 'user', 'content' => $prompt],
                    ],
                ]);
        ...
        return response()->json([
            'message' => 'Here are your flashcards, ready to be saved to the database.',
            'flashcards' => $flashcards
        ]);
    }
    ...
    // public function get_flashcards(Request $request)
    // {
    //     $validator = Validator::make($request->all(), [
    //         'lesson_id' => 'required|exists:lessons,id',
    //     ]);
