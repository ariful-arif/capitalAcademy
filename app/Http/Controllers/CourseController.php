...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'is_certificate_course' => Rule::in(['1', '0']),
            'certificate_course_type' => 'required_if:is_certificate_course,1|min:1|nullable',
        ]);
        ...
        $data['is_certificate_course']          = $request->is_certificate_course;
        $data['certificate_course_type']            = $request->certificate_course_type;
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->tab == 'basic') {
            ...
            $data['status']            = $request->status;
            $data['instructor_ids']       = json_encode($request->instructors);
        } elseif ($request->tab == 'certificate_program') {
            $rules = [
                'is_certificate_course' => Rule::in(['1', '0']),
                'certificate_course_type' => 'required_if:is_certificate_course,1|min:1|nullable',
            ];
            ...
            $data['certificate_course_type']            = $request->certificate_course_type;
            // $certificate_ids = $request->certificate_ids;
            $certificateIds = $request->certificate_ids; // Array of certificate IDs
            // $newCourseId = $request->course_id; // Course ID to add
            ...
            foreach ($certificateIds as $certificateId) {
                ...
                if ($certificate) {
                    // Decode existing course_ids
                    $existingCourseIds = json_decode($certificate->course_ids, true) ?? [];

                    // Ensure it's an array and add the new course ID if it's not already there
                    if (!in_array($id, $existingCourseIds)) {
                        $existingCourseIds[] = $id;
                    }

                    // Update the course_ids field
                    $certificate->course_ids = json_encode($existingCourseIds);
                    $certificate->save();
                }
            }
    ...
    public function delete($id)
    {
        ...
        // Remove the course from all certificate programs
        $certificatePrograms = CertificateProgram::whereJsonContains('course_ids', (string)$id)->get();

        foreach ($certificatePrograms as $certificate) {
            $courseIds = json_decode($certificate->course_ids, true);

            // Remove the deleted course ID
            $updatedCourseIds = array_filter($courseIds, fn($courseId) => $courseId != $id);

            // Update certificate program
            $certificate->update(['course_ids' => json_encode(array_values($updatedCourseIds))]);
        }
