...
if (!function_exists('count_newsrooms_by_category')) {
    function count_newsrooms_by_category($category_id = "")
    {
        if ($category_id != '') {
            $total_newsroom = DB::table('newsrooms')->where('status', 1)->where('category_id', $category_id)->count();
            return $total_newsroom;
        }
    }
}

if (!function_exists('get_newsroom_category_name')) {
    function get_newsroom_category_name($id = "")
    {
        if ($id != '') {
            $category_title = DB::table('newsroom_categories')->where('id', $id)->value('title');
            return $category_title;
        }
    }
}
if (!function_exists('count_learnings_by_category')) {
    function count_learnings_by_category($category_id = "")
    {
        if ($category_id != '') {
            $total_learning = DB::table('learnings')->where('status', 1)->where('category_id', $category_id)->count();
            return $total_learning;
        }
    }
}

if (!function_exists('get_learning_category_name')) {
    function get_learning_category_name($id = "")
    {
        if ($id != '') {
            $category_title = DB::table('learning_categories')->where('id', $id)->value('title');
            return $category_title;
        }
    }
}
...
if (!function_exists('get_dynamic_pages_settings')) {
    function get_dynamic_pages_settings($type = "", $return_type = false)
    {
        $value = DB::table('dynamic_pages')->where('key', $type);
        if ($value->count() > 0) {
            if ($return_type === true) {
                return json_decode($value->value('value'), true);
            } elseif ($return_type === "object") {
